...
for _profile_name, _weights in WEIGHT_PROFILES.items():
    assert (
        abs(sum(_weights.values()) - 1.0) < 1e-6
    ), f"Weight profile '{_profile_name}' does not sum to 1.0"

_SALES_TITLE_KEYWORDS = [
    "sales",
    "account executive",
    "account manager",
...
def get_profile(name: str) -> Dict[str, float]:
    if name not in WEIGHT_PROFILES:
        logger.warning(
            f"Unknown weight profile '{name}'. Falling back to '{DEFAULT_PROFILE}'."
        )
...
def suggest_profile(job_title: str, industry: Optional[str] = None) -> str:
    ...
    if any(
        keyword_found(keyword, normalized_title) for keyword in _SALES_TITLE_KEYWORDS
    ):
        return "sales"
    if any(
        keyword_found(keyword, normalized_title) for keyword in _DESIGN_TITLE_KEYWORDS
    ):
        return "design"
    ...
    if industry:
        ...
        if any(
            keyword_found(keyword, normalized_industry)
            for keyword in _SALES_INDUSTRY_KEYWORDS
        ):
            return "sales"
        if any(
            keyword_found(keyword, normalized_industry)
            for keyword in _DESIGN_INDUSTRY_KEYWORDS
        ):
            return "design"
